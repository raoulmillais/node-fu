...
if (args.length < 5) {
	throw new Error('Incorrect number of arguments, expected project ' +
...
fs.readFile(args[4], function(err, data) {
	...
	parsed.homepage.push('http://github.com/' +
						githubUsername + '/' + projectName);
	parsed.repository.url = 'git://github.com/' + githubUsername + '/' +
							projectName + '.git';
	parsed.author = gitUser + ' <' + gitEmail + '>';

	if (args[5] && args[5] =='web') {
		parsed.dependencies = {
			"express": "2.4.5",
			"now": "0.7.4",
			"browserify": "1.4.3",
